...
Accounts serve as a user-controlled smart wallet from which the user can perform a multitude of actions.

Arcadia Accounts are owned by one specific user, the owner of the Account.
...
The owner of the Account holds many privileges, such as being able to deposit and withdraw assets,
use the assets to interact with other DeFi protocols,
change the creditor (see further) or transfer the Account to another owner.

\subsection{Margin Accounts}
\label{subsec:margin-accounts}
Arcadia Accounts are an implementation of on-chain Margin Accounts.
Margin Accounts enable non-trusting entities to manage counter party risks of financial contracts.
An investor can use a margin Account for instance to increase their buying power, to short assets, or for financial contracts such as futures, swaps or options.

Each Margin Account is a separate user-owned smart contract that does the accounting of both the assets and liabilities of its owners.
Margin Accounts can be seen as an extension to standard smart wallets.
With the assets held by the Margin Account you can do similar actions as you can do with a standard wallet
but additionally you can take liabilities against the Account.

The Margin Account will enforce its margin requirements continuously.
This guarantees that the total value of all assets within the Account is always bigger as the liabilities against the Account (it is over-collateralized).
If a certain transaction would bring the Account at risk of becoming under-collateralized, the transaction will revert.

In the next paragraphs we will express the Margin requirements mathematically, introduce some margin related concepts.
...
We define the available margin, $am$, as the total value that the Account can use to secure open positions.
...
The free margin, $fm$, is the remaining amount of collateral value that is left in the account.
It is the amount of collateral value that can be withdrawn from the account, or it can be used to increase the open position.
...
The maintenance margin of an asset, $MM_{i}^{c}$, is also a term often used in traditional finance and similar to the initial margin.
...
There are three different account states, depending on the value of the Used Margin of the Account and the total value of the assets of the account.
...
\begin{enumerate}
    ...
    \item \textsc{Unhealthy state:} The Account is in an unhealthy state if the used margin is smaller than the Collateral Value but bigger than the Liquidation Value:
        ...
        Accounts in the unhealthy state can only be de-risked by or reducing the open position, or adding more collateral.
    ...
    \item \textsc{Liquidatable state:} The Account is in a liquidatable state if the used margin is smaller than the Liquidation Value:
        ...
        The account is now frozen and a liquidation process of the assets within the account start.
...
By allowing multiple types of assets to be deposited within a single Account, users, institutions and protocols can build diversified portfolio's and complex strategies.
